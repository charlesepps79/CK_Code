...
%LET TCI_LOC = 
"\\rmc.local\dfsroot\Dept\Marketing\Analytics\DigItal\Credit Karma\Input Lead and App Files\CK leads 20180730.xlsx";
%LET APPFILE_LOC = 
"\\rmc.local\dfsroot\Dept\Marketing\Analytics\DigItal\Credit Karma\Input Lead and App Files\ck_app_data_20180731.xlsx";
%LET CK_OUTPUT = 
...
DATA APPS;
	SET APPFILE; /* FORMAT FUNDEDDATE yymmddd10. */
	IF APPLICATIONENTERDATE <  '1JAN2018'D THEN DELETE;
	IF FUNDEDDATE = 'NULL' THEN FUNDEDDATE = "";
/*	FUNDEDDATE = INPUT(STRIP(FUNDEDDATE), mmddyy10.);*/
...
DATA MERGE1;
	...
	*** AS APPROVED HERE. TASKSTATUS OF PENDING APPROVAL --------- ***;
	...
	IF LoanNumber NOT IN ('None', '') THEN IS_FUNDED = 1;
	...
	ISPQAPP = 1;
	IF AMT_APPROVED > 0 AND REASONDECLINED = '' 
		THEN IS_APPROVE = 1;
	APP_BEFORE_LEAD = 0;
...
DATA LAST(
	KEEP = CK_TRACKING_ID LEADDATE APPLICATIONENTERDATE FUNDEDDATE 
	...
	FORMAT CK_TRACKING_ID LEADDATE APPLICATIONENTERDATE FUNDEDDATE
		   ISPQAPP IS_PREQUAL IS_APPL IS_APPROVE IS_FUNDED AMT_APPLIED 
		   AMT_APPROVED TERM_APPROVED RATE_APPROVED AMOUNT_FUNDED 
		   TERM_FUNDED RATE_FUNDED;
...
PROC EXPORT 
	DATA = LAST 
	OUTFILE =  "&CK_OUTPUT\CreditKarma_Regional_07312018.csv"
	DBMS = CSV
	REPLACE;
RUN;
...
PROC EXPORT 
	DATA = SUMMARY 
	OUTFILE = "&CK_SUMMARY\CKRPT_07312018.xlsx" 
	REPLACE ;
RUN;
